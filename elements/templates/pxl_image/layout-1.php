...
<?php if (!empty($settings['img_effect'])) {
    echo esc_attr($settings['img_effect']);
} ?> 
<?php echo esc_attr($settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms" <?php if ($settings['img_effect'] == 'pxl-image-tilt') : ?>data-maxtilt="<?php echo esc_attr($settings['max_tilt']); ?>" data-speedtilt="<?php echo esc_attr($settings['speed_tilt']); ?>" data-perspectivetilt="<?php echo esc_attr($settings['perspective_tilt']); ?>" <?php endif; ?> <?php if ($settings['img_effect'] == 'pxl-parallax-scroll') : ?>data-parallax='{"<?php echo esc_attr($settings['parallax_scroll_type']); ?>":<?php echo esc_attr($settings['parallax_scroll_value_x']); ?>}' <?php endif; ?>>
    <div class="pxl-item--inner <?php if ($settings['img_effect'] == 'pxl-image-border') : ?>wow<?php endif; ?>">
        <?php if ($settings['source_type'] == 's_img' && !empty($settings['image']['id']) || $settings['source_type'] == 'f_img' && has_post_thumbnail()) :
            ...
            <?php switch ($settings['image_type']) {
                ...
                default: ?>
                    ...
                    <?php if ($settings['show_ripple'] == 'true') {
                        echo 'hover-imge-ripple';
                    } ?>"
                        <?php if ($settings['show_ripple'] == 'true') {
                            echo 'data-image-url="' . esc_url($thumbnail_url) . '"';
                            echo 'style="background-image: url(' . esc_url($thumbnail_url) . ');"';
                        } ?> data-parallax-value="<?php echo esc_attr($settings['parallax_value']); ?>">
        ...
        <?php if ($settings['img_effect'] == 'pxl-image-border') : ?>
            <div class="pxl-divide--group">
                ...
                <div class="pxl-divider--box">
                    <div class="pxl-item--divider pxl-divider--left1"></div>
                </div>
