 ...
 * Swipper Lib
 */
if (!function_exists('frameflow_elements_scripts')) {
    add_action('wp_enqueue_scripts', 'frameflow_elements_scripts');
    function frameflow_elements_scripts()
    {
        ...
        wp_enqueue_script('pxl-ripple', get_template_directory_uri() . '/assets/js/libs/ripple.js', ['jquery'], $theme->get('Version'), true);
        wp_enqueue_script('pxl-matter', get_template_directory_uri() . '/assets/js/libs/matter.js', ['jquery'], $theme->get('Version'), true);
        wp_enqueue_script('frameflow-imagesloaded', get_template_directory_uri() . '/assets/js/libs/imagesloaded.min.js', ['jquery'], $theme->get('Version'), true);
        ...
        wp_register_script('pxl-bundled-lenis', get_template_directory_uri() . '/assets/js/libs/set1/bundled-lenis.min.js', array('jquery'), '1.0.0', true);

        wp_register_script('frameflow-particle', get_template_directory_uri() . '/elements/widgets/js/particle.js', ['jquery'], $theme->get('Version'), true);
        wp_register_script('frameflow-parallax', get_template_directory_uri() . '/elements/widgets/js/parallax.js', ['jquery'], $theme->get('Version'), true);
        ...
        wp_register_script('frameflow-counter', get_template_directory_uri() . '/elements/widgets/js/counter.js', ['jquery'], $theme->get('Version'), true);
        wp_register_script('frameflow-accordion', get_template_directory_uri() . '/elements/widgets/js/accordion.js', ['jquery'], $theme->get('Version'), true);
        wp_register_script('frameflow-tabs', get_template_directory_uri() . '/elements/widgets/js/tabs.js', ['jquery'], $theme->get('Version'), true);
        wp_register_script('pxl-countdown', get_template_directory_uri() . '/elements/widgets/js/pxl-countdown.js', ['jquery'], $theme->get('Version'), true);
        wp_enqueue_script('frameflow-elementor', get_template_directory_uri() . '/elements/widgets/js/elementor.js', ['jquery'], $theme->get('Version'), true);
    }
}
 ...
 */
function frameflow_get_post_grid_layout($pt_supports = [])
{
    $post_types  = frameflow_get_post_type_options($pt_supports);
    $result = [];
    if (!is_array($post_types))
        return $result;
    foreach ($post_types as $name => $label) {
        $result[] = array(
            'name'     => 'layout_' . $name,
            'label'    => sprintf(esc_html__('Select Template of %s', 'frameflow'), $label),
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => frameflow_get_grid_layout_options($name),
...
function frameflow_get_grid_layout_options($post_type_name)
{
    $option_layouts = [];
    switch ($post_type_name) {
        ...
        case 'event':
            $option_layouts = [
                ...
                'event-2' => [
                    'label' => esc_html__('Layout 2', 'frameflow'),
                    'image' => get_template_directory_uri() . '/elements/widgets/img-layout/pxl_post_grid/event-layout2.jpg'
                ],
            ];
            break;  
    }
...
function frameflow_get_term_by_post_type($pt_supports = [], $args = [])
{
    ...
    foreach ($post_types as $name => $label) {
        ...
        $result[] = array(
            'name' => 'source_' . $name,
            'label' => sprintf(esc_html__('Select Term', 'frameflow'), $label),
            'description' => esc_html__('Get all when no term selected', 'frameflow'),
...
function frameflow_get_grid_term_by_post_type($pt_supports = [], $args = [])
{
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types  = frameflow_get_post_type_options($pt_supports);
    $result = [];
    ...
    foreach ($post_types as $name => $label) {
        ...
        $result[] = array(
            'name'     => 'source_' . $name,
            'label'    => sprintf(esc_html__('Select Term of %s', 'frameflow'), $label),
            'type'     => \Elementor\Controls_Manager::SELECT2,
            'multiple' => true,
            'options'  => pxl_get_grid_term_options($name, $taxonomy),
            'condition' => array_merge(
...
function frameflow_get_ids_by_post_type($pt_supports = [], $args = [])
{
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types = frameflow_get_post_type_options($pt_supports);
    $result = [];
    ...
    foreach ($post_types as $name => $label) {
        ...
        $posts = frameflow_list_post($name, false);

        $result[] = array(
            'name' => 'source_' . $name . '_post_ids',
            'label' => sprintf(esc_html__('Select posts', 'frameflow'), $label),
            'type' => \Elementor\Controls_Manager::SELECT2,
            'multiple' => true,
            'options' => $posts,
            'label_block' => true,
...
function frameflow_get_ids_unselected_by_post_type($pt_supports = [], $args = [])
{
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types = frameflow_get_post_type_options($pt_supports);
    $result = [];
    ...
    foreach ($post_types as $name => $label) {
        ...
        $result[] = array(
            ...
            'label' => sprintf(esc_html__('Unselected posts', 'frameflow'), $label),
            'type' => \Elementor\Controls_Manager::SELECT2,
            'multiple' => true,
            'options' => $posts,
            'label_block' => true,
...
function frameflow_get_grid_ids_by_post_type($pt_supports = [], $args = [])
{
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types = frameflow_get_post_type_options($pt_supports);
    $result = [];
 ...
 */
function frameflow_get_post_carousel_layout($pt_supports = [])
{
    $post_types  = frameflow_get_post_type_options($pt_supports);
    $result = [];
    if (!is_array($post_types))
    ...
    foreach ($post_types as $name => $label) {
        $result[] = array(
            ...
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => frameflow_get_carousel_layout_options($name),
...
function frameflow_get_carousel_layout_options($post_type_name)
{
    $option_layouts = [];
    switch ($post_type_name) {
        case 'portfolio':
 ...
 */
function frameflow_get_post_list_layout($pt_supports = [])
{
    $post_types  = frameflow_get_post_type_options($pt_supports);
    $result = [];
    if (!is_array($post_types))
    ...
    foreach ($post_types as $name => $label) {
        $result[] = array(
            ...
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => frameflow_get_list_layout_options($name),
            'prefix_class' => 'post-layout-',
            'condition' => [
                'post_type' => [$name]
            ]
...
function frameflow_get_list_layout_options($post_type_name)
{
    $option_layouts = [];
    switch ($post_type_name) {
        case 'post':
...
function frameflow_get_carousel_term_by_post_type($pt_supports = [], $args = [])
{
    $args = wp_parse_args($args, ['condition' => 'post_type', 'custom_condition' => []]);
    $post_types  = frameflow_get_post_type_options($pt_supports);
    $result = [];
 ...
 */
function frameflow_get_post_modern_layout($pt_supports = [])
{
    $post_types  = frameflow_get_post_type_options($pt_supports);
    $result = [];
    if (!is_array($post_types))
    ...
    foreach ($post_types as $name => $label) {
        $result[] = array(
            ...
            'type'     => 'layoutcontrol',
            'default' => 'post-1',
            'options'  => frameflow_get_modern_layout_options($name),
            'prefix_class' => 'post-layout-',
            'condition' => [
                'post_type' => [$name]
            ]
...
function frameflow_get_modern_layout_options($post_type_name)
{
    $option_layouts = [];
    switch ($post_type_name) {
