...
function frameflow_fonts_url()
{
    ...
    if ('off' !== _x('on', 'Boldonse font: on or off', 'frameflow')) {

        $fonts[] = 'Boldonse&display=swap';
    }

    if ('off' !== _x('on', 'Geist font: on or off', 'frameflow')) {
...
?>
    <<?php echo '' . $tag ?> <?php comment_class(empty($args['has_children']) ? '' : 'parent') ?> id="comment-<?php comment_ID() ?>">
                    ...
                    <div class="comment-holder">
                        ...
                        <div class="comment-meta ">
                            ...
                            <span class="comment-date">
                                <?php echo get_comment_date('M d, Y') . ' - ' . get_comment_time(); ?>
                            </span>
        ...
        add_action('admin_post_submit_feedback_form', 'handle_feedback_form');
        add_action('admin_post_nopriv_submit_feedback_form', 'handle_feedback_form'); // For non-logged-in users

        function handle_feedback_form()
        {
            // Check if the necessary fields are set
            if (isset($_POST['name'], $_POST['email'], $_POST['rating'], $_POST['feedback'])) {

                // Verify Nonce
                if (!isset($_POST['feedback_nonce']) || !wp_verify_nonce($_POST['feedback_nonce'], 'submit_feedback_form')) {
                    wp_die(esc_html__('Security check failed (Nonce verification).', 'frameflow'));
                }
                ...
                $rating   = intval($_POST['rating']); // Sanitize rating (ensure it's an integer)
                $feedback = sanitize_textarea_field($_POST['feedback']);

                // Validate email
                if (! is_email($email)) {
                    wp_redirect(home_url('/feedback-error/'));
                    exit;
                }
        ...
        function frameflow_mouse_move_animation()
        {
            ...
            if ($mouse_move_animation) {
                wp_enqueue_script('frameflow-cursor', get_template_directory_uri() . '/assets/js/libs/cursor.js', array('jquery'), '1.0.0', true); ?>
