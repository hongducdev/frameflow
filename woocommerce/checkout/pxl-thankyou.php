...
if ($order) :
    ...
    // If order has event tickets, use event-specific thank you page
    if ($has_event_tickets) {
        $event_template = get_template_directory() . '/woocommerce/checkout/pxl-thankyou-event.php';
        ...
        <div class="thankyou-page--content">
            ...
            <div class="order-overview">
                ...
                <div class="order-status d-flex align-items-baseline justify-content-center">
                    ...
                    <span class="status shipped d-flex align-items-center">
                        ...
                        <?php esc_html_e('Shipped', 'frameflow'); ?>
                        <?php
                        $confirmed_date = $order->get_date_created();
                        if ($confirmed_date) {
                            echo '<small>' . esc_html($confirmed_date->date('j M Y')) . '</small>';
                    ...
                    <span class="status delivered d-flex align-items-center">
                        ...
                        <?php esc_html_e('Delivered', 'frameflow'); ?>
                        <?php
                        $delivered_date = $order->get_meta('_delivery_date');
                        if ($delivered_date && strtotime($delivered_date)) {
                            echo '<small>(' . esc_html(date('F j, Y', strtotime($delivered_date))) . ')</small>';
                        }
                        ?>
                    </span>
                </div>
            ...
            <div class="address-section">
                <div class="shipping-address">
                    <h3><?php esc_html_e('Shipping address', 'frameflow'); ?></h3>
        ...
        <div class="thankyou-page--sidebar">
            ...
            <div class="order-items">
                ...
                <div class="order-summary">
                    ...
                    <p><strong><?php esc_html_e('Shipping:', 'frameflow'); ?></strong>
            ...
            <div class="feedback-form">
                <h3><?php esc_html_e('Give us a feedback', 'frameflow'); ?></h3>
                <p><?php esc_html_e('Let us know what you think about the shopping experience, and get a gift coupon for the next shopping.', 'frameflow'); ?></p>
                <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="post">
                    <?php wp_nonce_field('submit_feedback_form', 'feedback_nonce'); ?>
                    <input type="hidden" name="action" value="submit_feedback_form">
                    <input type="hidden" name="order_id" value="<?php echo esc_attr($order->get_id()); ?>">
                    <input type="text" name="name" required placeholder="<?php esc_attr_e('Name', 'frameflow'); ?>">
                    <input type="email" name="email" required placeholder="<?php esc_attr_e('Email', 'frameflow'); ?>">
                    <label for="rating"><?php esc_html_e('How was your experience?', 'frameflow'); ?></label>
                    ...
                    <textarea name="feedback" required placeholder="<?php esc_attr_e('Share your experience...', 'frameflow'); ?>"></textarea>
                    <button type="submit"><?php esc_html_e('Send', 'frameflow'); ?></button>
                </form>
                <div class="social-links">
                    <h3><?php esc_html_e('Share the love', 'frameflow'); ?></h3>
...
<?php if ($order) : ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            ?>
                var shippingAddress = "<?php echo esc_js($shipping_address); ?>";
                ...
                fetch(geocodeUrl)
                    ...
                    .then(data => {
                        if (data.length > 0) {
                            ...
                            L.marker([lat, lon]).addTo(map).bindPopup('<?php echo esc_js(__('Shipping Address', 'frameflow')); ?>').openPopup();
                        } else {
                            console.warn("Address not found for geocoding.");
                        }
                    })
                    .catch(error => {
            ...
            <?php else : ?>
                console.warn("Shipping address is empty.");
            <?php endif; ?>
            ...
            if (twitterShare) {
                twitterShare.href = 'https://twitter.com/intent/tweet?url=' + encodeURIComponent(window.location.href) + '&text=' + encodeURIComponent(document.title);
            }
            if (linkedinShare) {
                linkedinShare.href = 'https://www.linkedin.com/shareArticle?mini=true&url=' + encodeURIComponent(window.location.href);
            }
            if (tiktokShare) {
                tiktokShare.href = 'https://www.tiktok.com/share?url=' + encodeURIComponent(window.location.href);
            }
        });
    </script>
<?php endif; ?>